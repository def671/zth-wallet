...
const fs = require("fs");
const os = require("os");
// const datadir = process.env.GETH_DATADIR || path.resolve(process.env.HOME || '~', '.zthdata');
...
startGeth() {
  // get the path of geth and execute the child process
  try {
    this.isRunning = true;
    const gethPath = path.join(this.binaries, os.type() === "Windows_NT" ? "geth.exe" : "geth");
    this.gethProcess = child_process.spawn(gethPath, [
      "--datadir",
      path.resolve(datadir),
      "--log.file",
      path.resolve(datadir, "geth.log"),
    ...
    if (!this.gethProcess) {
      ...
      this.gethProcess.on("close", () => {
        if (this.isRunning) {
          dialog.showErrorBox("Error running the node", "The node stopped working. The Wallet will close!");
          app.quit();
        }
      });
